export const pythonCodeTwo = `
# Copyright @ 2020 Thought Machine Group Limited. All rights reserved.
display_name = "Fin Credit Card Demo5"
api = "3.8.0"
version = "1.1.14"
tside = Tside.ASSET
...
parameters = [
    ...
    Parameter(
        ...
        "(i.e. an active Interest Free Period or transaction types that always charge interest from the transaction date), "
        "determines the start point for interest accrual on transactions charged when entering Revolver status. "
        "For more informantion please refer to the product documentation.",
    ...
    Parameter(
        ...
        shape=StringShape,
        default_value=json_dumps({"balance_transfer": {"REF1": "0.04", "REF2": "0.04"}}),
        update_permission=UpdatePermission.USER_EDITABLE_WITH_OPS_PERMISSION,
    ),
    Parameter(
    ...
    Parameter(
        ...
        default_value=OptionalValue("INTEREST_WRITEOFF"),
        update_permission=UpdatePermission.USER_EDITABLE_WITH_OPS_PERMISSION,
    ),
    Parameter(
        ...
        level=Level.INSTANCE,
        shape=OptionalShape(AccountIdShape),
        default_value=OptionalValue("PRINCIPAL_WRITEOFF"),
        update_permission=UpdatePermission.USER_EDITABLE_WITH_OPS_PERMISSION,
    ),
    Parameter(
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will suspend the"
        "internal address transfers from billed to unpaid balances on PDD",
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will suspend the"
        "internal address updates on PDD that age the overdue balance buckets",
    ...
    Parameter(
        ...
        shape=StringShape,
        level=Level.INSTANCE,
        default_value=json_dumps({"purchase": "",  "transfer": ""}), 
        update_permission=UpdatePermission.USER_EDITABLE_WITH_OPS_PERMISSION,
    ),
    Parameter(
    ...
    Parameter(
        name="mab_annual_fee_cap",
        level=Level.INSTANCE,
        description="Annual card Fee is charged only if Monthly Average Balance is less than specified amount",
        display_name="Monthly Average Balance",
        shape=MoneyShape,
...
def execution_schedules():
    ...
    local_account_creation_dt = _localize_datetime(account_creation_dt)
    annual_fee_day = (
            "last"
    ...
    annual_fee_schedule = _convert_to_utc_schedule(
        {
            "year": str(account_creation_dt.year + 1),
...
def _autopay_override(postings):
    for post in postings:
        if "AUTOPAY" in post.client_transaction_id:  
            return True
        else:
            return False
...
def _zero_out_balances_for_account_closure(vault, effective_date, in_flight_balances, txn_types):
    ...
    Create postings to zero out remaining balances that aren't written off or paid-back when an
     account is being closed or written off. All balances other than AVAILABLE_BALANCE,
     <transaction_type>_INTEREST_UNCHARGED, <transaction_type>_INTEREST_FREE_PERIOD_INTEREST_UNCHARGED
     and INTERNAL must be 0 as the full outstanding balance
     has either been paid off or written off
    :param vault:
...
def _process_write_off(denomination, in_flight_balances, effective_date, instructions, vault):
    ...
    principal_write_off_account = _get_parameter(
        vault, name=PRINCIPAL_WRITE_OFF_INTERNAL_ACCOUNT, optional=True
    )
    interest_write_off_account = _get_parameter(
        vault, name=INTEREST_WRITE_OFF_INTERNAL_ACCOUNT, optional=True
    )
    if not (principal_write_off_account and interest_write_off_account):
        raise Rejected(
            "Write-off internal accounts not populated",
...
def _check_txn_type_parameter_configuration(vault, effective_timestamp=None):
    ...
    def _compare_parameter_keys(
        ...
        :param main_list: set, set of keys to check against
        :param compare_list_name: string, name of a parameter to check against main_list
        :param effective_timestamp: timestamp at which to fetch the parameter with compare_list_name
        :param match_type: Optional string, 'match' - keys in the compare_list_name parameter must exactly match main_list
        ...
        :param values: Boolean, whether to use the values of the compare_list, rather than the keys to match with
    ...
    # These don't need to have all types included, but any here must be present in supported_txn_types
    _compare_parameter_keys(
...
def _check_account_has_sufficient_funds(vault, balances, denomination, postings):
    ...
    _update_balances(vault.account_id, postings_balances, postings, tside)
...
def _check_txn_type_time_limits(vault, postings, effective_date):
    ...
    txn_type_limits = _get_parameter(
...
def _make_repayment_gl_postings(
    ...
    :param remaining_credit_limit: Decimal, remaining credit line that hasn't been spent, normally +ve
...
def _create_postings(
    ...
    if in_flight_balances:
        _update_balances(vault.account_id, in_flight_balances, new_instructions, tside)
...
def _calculate_aggregate_balance(
    ...
    offset = 0
    if include_deposit:
        # The deposit balance is subtracted as it's money owed to the customer rather than owed by them
...
def _make_deposit_postings(
    ...
    if is_repayment:
        ...
        to_address = DEFAULT_ADDRESS
        action = f"CUSTOMER_TO_OTHER_LIABILITY_GL"
    else:
        from_account_id = other_liability_account
        from_address = DEFAULT_ADDRESS
        to_account_id = vault.account_id
        to_address = INTERNAL_BALANCE
        action = f"OTHER_LIABILITY_TO_CUSTOMER_GL"
...
def _get_txn_type_and_ref_from_posting(
    ...
    instruction_details,
    effective_timestamp,
    supported_txn_types=None,
    txn_code_to_type_map=None,
    upper_case_type=False,
):
    """
    ...
    :param vault:
    :param instruction_details: Dict[str, str], posting metadata for get the transaction code from
    :param effective_timestamp: datetime, timestamp to get required parameters as-of
    :param supported_txn_types: Optional[Dict[str, Optional[List[str]]]], the contract's supported
    transaction types
    :param txn_code_to_type_map: Optional[Dict[str, str]], map of transaction type code -> transaction_type
    :param upper_case_type: Optional[bool], if true we will return txn_type in upper case
    :return: (str, str), the transaction type and the reference (which may be None)
    """
...
def _accrue_interest(
    ...
    # will be zeroed out on PDD only if MAD is repaid. If MAD is not repaid, we will charge the accrued interest.
...
def _get_balances_to_accrue_on(
    ...
    :param denomination: str, the account denomination
    :param supported_txn_types: Dict[str, List[str]], the transaction types supported by the account
    :param supported_fee_types: List[str], the fee types supported by the account
    :param txn_types_to_charge_interest_from_txn_date: List[str], list of transaction types that
    get charged interest straight away from the date of transaction
    :param accrue_interest_on_unpaid_interest: bool, if true interest is accrued on unpaid interest
    :param accrue_interest_on_unpaid_fees: bool, if true interest is accrued on unpaid fees
    :param txn_types_in_interest_free_period: Dict[str, Optional[List[str]]], transaction types
    and refs that have an active interest free period
    :param accrue_interest_from_txn_day: boolean, marks the interest accrual approach in this product
    :return: Dict[str, Dict[str, Decimal]], map of transaction type -> map of transaction ref
    ('' if no ref) -> balance to accrue on
    """
    ...
    def _update_balances_to_accrue_on(charge_type, sub_type, ref=None):
        ...
        # make sure to mark that interest needs to be accrued to the POST_SCOD or PRE_SCOD interest uncharged balance addresses
        # respectively from a <transaction_type>_BILLED or <transaction_type>_CHARGED address IF
    ...
    # Accrue interest on all balances if we are in revolver, or if the account is projected to go into revolver by next PDD
    # We will decide further down whether the interest is accrued to UNCHARGED address or CHARGED directly
...
def _calculate_accruals_and_create_instructions(
    ...
    def _create_accrual_instructions(charge_type, sub_type, amount, ref, accrual_type):
        ...
        :param accrual_type: str, either '', PRE_SCOD or POST_SCOD applicable for interest accrual to _UNCHARGED balances from txn day
        """
        ...
        if accrual_amount > 0:
            ...
            if is_txn_type_in_interest_free_period:
                ...
                # We do not attempt to charge interest on transactions that have active interest free periods even in revolver
                _set_accruals_by_sub_type(
                    interest_by_charge_sub_and_accrual_type,
                    charge_type,
                    sub_type,
            ...
            # If the account is in revolver or if the txn type has charge_interest_from_transaction_date as True,
            # interest will be charged immediately unless the txn type has an active interest free period.
...
def _make_accrual_posting(
    ...
    :param vault:
    :param accrual_amount: Decimal, amount to accrue
    :param denomination: str, denomination to accrue in
    :param stem: str, transaction type being accrued on (with reference appended if present)
    :param instructions: List[PostingInstruction], to be extended with accrual posting instructions
    :param instruction_details: Dict[str, str], metadata to attach to the postings
    :param reverse: bool, used to reverse accrual postings (e.g. zeroing out accrued interest)
    :param accrual_type: Optional[str], either '' or PRE_SCOD or POST_SCOD applicable for interest accrual to _UNCHARGED balances from txn day
    :return: None
    """
    action = f"ACCRUE_INTEREST" if not reverse else f"REVERSE_UNCHARGED_INTEREST"
    stem_with_accrual_type = f"{stem}_{accrual_type}" if accrual_type else stem
    _make_internal_address_transfer(
...
def _get_first_scod(account_creation_date, localize_datetime=False):
    ...
    local_scod_start = local_account_creation_date + timedelta(months=1) - timedelta(days=3)
...
def _is_between_pdd_and_scod(
    ...
    There is no PDD in the first statement cycle, we will return false if current_date is within first statement cycle.
    :param vault:
...
def _process_statement_cut_off(vault, effective_date, in_flight_balances=None, is_final=False):
    ...
    # up until that point. For example, if we assume the schedule is run at SCOD+1T00:00:00, all balances
    # up until SCODT23:59:59.99999 are included in the statement. This includes interest accrual
    if is_final:
...
def _adjust_interest_uncharged_balances(
    ...
    <transaction_type>_INTEREST_PRE_SCOD_UNCHARGED moves to <transaction_type>_INTEREST_POST_SCOD_UNCHARGED
    :param vault:
...
def _handle_live_balance_changes(vault, denomination, cut_off_dt, instructions_ts, is_pdd=False):
    ...
    for instructions in instructions_ts.values():
        _update_balances(vault.account_id, in_flight_live_balances, instructions, tside)
...
def _publish_statement_workflow(vault, statement_end, final_statement, mad, is_final):
    ...
    # update SCOD schedule
    vault.update_event_type(
        event_type=EVENT_SCOD,
        schedule=EventTypeSchedule(
            month=str(next_scod_start.month),
            day=str(next_scod_start.day),
            hour=str(next_scod_start.hour),
            minute=str(next_scod_start.minute),
            second=str(next_scod_start.second),
        ),
    )
...
def _charge_interest(
    ...
    for charge_sub_and_accrual_type, ref_to_amount in accruals_to_charge.items():
        ...
        for ref, amounts in ref_to_amount.items():
            # If there is an active interest free period and _charge_interest is called by EVENT_ACCRUE, i.e. is_pdd=False,
            # we don't want to charge interest for the txn type, instead we will make sure it's accrued as uncharged interest,
            # to be realised or wiped out during PDD.
...
def _process_payment_due_date(vault, effective_date):
    ...
    # In all cases, we will want to have all INTEREST_FREE_PERIOD_INTEREST_UNCHARGED addresses taken off on PDD
    ...
    # If not revolving and the full statement balance was paid off, all uncharged interest should be zeroed out
    ...
    if not _is_revolver(in_flight_balances, denomination):
        ...
        for dimensions, amount in in_flight_balances.items():
            ...
            if address.endswith(INTEREST_FREE_PERIOD_UNCHARGED_INTEREST_BALANCE):
                ...
                # For reference based transactions - BALANCE_TRANSFER_REF1_INTEREST_POST/PRE_SCOD_UNCHARGED
                charge_type = PRINCIPAL
                if accrue_interest_from_txn_day:
                    # to accommodate the PRE/POS_SCOD part of the address, build out the appropriate address type below
                    for accrual_type in ACCRUAL_TYPES:
                        if address.endswith(f"{accrual_type}_{UNCHARGED}"):
                            # If entering revolver, both PRE & POST address are moved to INTEREST_CHARGED
    ...
    if _is_flag_in_list_applied(vault, MAD_EQUAL_TO_ZERO_FLAGS, effective_date):
        ...
        for dimensions, amount in in_flight_balances.items():
            ...
            if address.endswith(INTEREST_FREE_PERIOD_UNCHARGED_INTEREST_BALANCE):
                # These balance addresses look like this:
                # For non-reference based transactions - PUCHASE_INTEREST_FREE_PERIOD_INTEREST_UNCHARGED
                # For reference based transactions - BALANCE_TRANSFER_REF1_INTEREST_FREE_PERIOD_INTEREST_UNCHARGED
                sub_type, ref = _get_txn_type_and_ref_from_address(
                    address,
                    supported_txn_types.keys(),
                    INTEREST_FREE_PERIOD_UNCHARGED_INTEREST_BALANCE,
        ...
        # The interest accrued on the INTEREST_FREE_PERIOD_INTEREST_UNCHARGED address will now be charged
        # Supply charge_interest_free_period=True into our _charge_interest call to ensure we don't get duplicated
        # client transaction IDs out of the _charge_interest call in the revolver logic.
        _charge_interest(
            vault,
            is_revolver=True,
            denomination=denomination,
...
def _update_overdue_buckets(vault, overdue_total, in_flight_balances, denomination, instructions):
    """
    Cycles existing overdue buckets and populates latest overdue bucket if required.
    :param vault:
    :param overdue_total: Decimal, overdue amount for this cycle, i.e. MAD not covered by repayments (which are both -ve)
    :param in_flight_balances: DefaultDict[Tuple[str, str, str, Phase], Balance], latest account
     balances updated with posting instructions created within the current hook execution
    :param denomination: str, account denomination
    :param instructions: List[PostingInstruction], posting instructions to be extended with
...
def _interest_address(txn_type, interest_status, txn_ref=None, accrual_type=None):
    ...
    # Account for transaction types with Interest Free period enabled and reflected in the txn_type variable
    ...
    # Construct the full address by filtering out the attributes which have a value of None (weren't passed in the function)
...
def _get_parameter(
    ...
    :param upper_case_dict_values: Optional boolean, if is_json is True and we are expecting the
    parameter to take shape Dict[str:Dict[str,str]], we will convert the Dict[str,str] values to upper case
    :param upper_case_list_values: Optional boolean, if is_json is True and we are expecting the
...
def _update_balances(account_id, balances, postings, contract_tside=Tside.ASSET):
    """
    ...
    :param contract_tside: Tside, tside for the account question. Optional if postings is not used
    :return: None
    """
...
def _get_txn_type_and_ref_from_address(address, base_txn_types, address_type):
    ...
    :param address_type: str, the string at the end of address that we slice off to obtain our TXN_TYPE_REF stem
    ...
    # We should have found a matching txn_type by now, but return original stem if we failed to do so.
    return stem, None
...
def _set_accruals_by_sub_type(
    accruals_by_sub_type, charge_type, sub_type, ref, accrual_amount, accrual_type=None
):
    """
    Construct accruals information based on charge_type, sub_type, accrual_type, ref, and accrual_amount

    :param accruals_by_sub_type: Dict[(str, str, str): Dict[str, str]], the dictionary that we add information to
    Examples:
    {(PRINCIPAL, 'balance_transfer', ''): {'REF1': 1.23, 'REF2': 4.56}}
    {(PRINCIPAL, 'purchase', 'PRE_SCOD'): {'': 7.89}}

    :param charge_type: str, either PRINCIPAL or FEE
    :param sub_type: str, the transaction type which doesn't contain a reference
    :param ref: Optional[str], the transaction reference if there is one
    :param accrual_amount: decimal, the amount to accrue
    :param accrual_type: Optional[str], either '' or PRE_SCOD or POST_SCOD applicable for interest accrual to _UNCHARGED balances from txn day
    """
...
def _is_txn_interest_accrual_from_txn_day(vault, at_dt=None) -> bool:
    """
    Returns current state of the accrue_interest_from_txn_day flag.

    :param vault:
    :param at_dt: Optional[datetime], the date time at which param needs to be returned, defaults to latest
    :return: bool
    """
...
def _is_apply_annual_fees(vault, denomination: str, effective_date: datetime, addresses: List[str] = None):
    """
    GenZ-Determine the monthly mean balance on the account. If monthly mean balance falls less than free cap then charge annual card fee else do not charge annual card fee if the user maintains monthly average balance above or equal to free cap.
    """
    creation_date = vault.get_account_creation_date()
    temp=effective_date - creation_date
    num_of_days=temp.days
    is_apply_annual_fees = False
    mab_annual_fee_cap = _get_parameter(name="mab_annual_fee_cap", vault=vault) 

    while num_of_days > 0:
        monthly_mean_balance, next_effective_date = _monthly_mean_balance(vault, denomination, effective_date)
        if monthly_mean_balance < mab_annual_fee_cap:
           is_apply_annual_fees = True
    return is_apply_annual_fees
     
def _monthly_mean_balance(
    vault, denomination: str, effective_date: datetime, addresses: List[str] = None
):
    """
    GenZ-Determine the average combined balance for the preceding month. The sampling period is from one
    ...
    return total / num_days, period_start



def _get_preceeding_month_average_balance(
    vault, denomination: str, effective_date: datetime, addresses: List[str] = None
):
    """
    GenZ-Determine the average combined balance for the preceding month. The sampling period is from one
    ...
    """
    creation_date = vault.get_account_creation_date()
    addresses = addresses or [FULL_OUTSTANDING_BALANCE]
